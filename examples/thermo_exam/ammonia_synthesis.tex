...
REACTOR = Pick.pick_state(
    dict(
        P1={'default':8,'pick':{'between':[7,10],'round':-1}}, # bar
        T1={'default':500,'pick':{'between':[480,520], 'round':0}} # K
    )
)
T = REACTOR.T1 * ureg.K
P = REACTOR.P1 * ureg.bar
d = SandlerProps
...
If the reactor is operated at constant \py{f'{T:.0f~P}'} and \py{f'{P:.0f~P}'}, determine the equilibrium mole fractions of each species in the reactor effluent.  You can assume ideal-gas behavior, and that the simplified van't Hoff equation applies for the temperature dependence of the equilibrium constant.
...
At \py{f'{T:.0f~P}'}, the equilibrium constant for the reaction at the reference temperature $T_0$, followed by the equilibrium constant at the operating temperature $T$, are calculated as follows (via the simplified van't Hoff equation):
