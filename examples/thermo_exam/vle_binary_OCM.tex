...
comps = Pick.pick_state(dict(xbub={'default':0.08,'pick':{'pickfrom':[0.06, 0.07, 0.08, 0.09, 0.10],'round':3}},
                             ydew={'default':0.12,'pick':{'pickfrom':[0.10, 0.11, 0.12, 0.13, 0.14],'round':3}}))
...
AnsSet.register(idx, group=group, label=r'c \(x_{\rm az}\)', value=xazeo.m, formatter='{:.4f}')
...
ax.scatter([xazeo], [Pazeo.m], label='azeotrope', color='red')
ax.set_xlim([0, 1])
...
For the system of ethyl ethanoate(1)/$n$-heptane(2) at \py{f'{T:.2f~P}'},
\begin{itemize}
\item[] $\ln\gamma_1 = \py{f'{Aij:.3f}'} x_2^2$ \ \ \ \ \ \ \ \ \ \ \ $\ln\gamma_2 = \py{f'{Aij:.3f}'} x_1^2$,
\item[] $P_1^{\rm vap}$ = \py{f'{Pv[0]:.2f~P}'} \ \ \ \ \  $P_2^{\rm vap}$ = \py{f'{Pv[1]:.2f~P}'}.
\end{itemize}
Assume you can use low-pressure VLE (i.e., ``Modified Raoult's Law''):\\*[3mm]
$y_iP = x_i\gamma_iP_i^{\rm vap}$\\*[3mm]
and answer the following.
\begin{enumerate}
\item[(a)] Compute the bubble-point pressure at $T$ = \py{f'{T:.2f~P}'}, $x_1$ = \py{f'{xbub:.2f}'}.
\item[(b)] Compute the dew-point pressure at $T$ = \py{f'{T:.2f~P}'}, $y_1$ = \py{f'{ydew:.2f}'}.
\item[(c)] What is the azeotrope composition and pressure at $T$ = \py{f'{T:.2f~P}'}?
\end{enumerate}
...
\begin{align*}
    P & = x_1 \gamma_1 P_1^{\rm vap} + x_2 \gamma_2 P_2^{\rm vap}\\
     ...
     & = (\py{f'{xbub:.2f}'})(\py{f'{g[0]:.4f}'})(\py{f'{Pv[0]:.2f~L}'}) + (1-\py{f'{xbub:.2f}'})(\py{f'{g[1]:.4f}'})(\py{f'{Pv[0]:.2f~L}'}) = \fbox{\py{f'{Pbub:.2f~L}'}}.
\end{align*}
...
So the value of $x$ that satisfies the implicit equation is \py{f'{xdew:.4f}'}.  This is the composition of the droplet of dew that condenses from a vapor with composition $y_1$ = \py{f'{ydew:.2f}'} at \py{f'{T:.2f~P}'} K.  The pressure at which this condensation occurs is the dew-point pressure:
\begin{align*}
    P & = x_1\gamma_1P_1^{\rm vap} + x_2 \gamma_2 P_2^{\rm vap}\\
    & = (\py{f'{xdew:.4f}'})(\py{f'{gdew[0]:.2f}'})(\py{f'{Pv[0]:.2f~L}'}) + (1-\py{f'{xdew:.4f}'})(\py{f'{gdew[1]:.2f}'})(\py{f'{Pv[1]:.2f~L}'})\\
    & = \fbox{\py{f'{Pdew:.2f~P}'}}.
\end{align*}
...
\begin{align*}
    ...
    & = \frac{1}{2}-\frac{1}{2(\py{Aij})}\ln\left(\frac{\py{f'{Pv[1]:.2f~P}'}{\py{f'{Pv[0]:.2f~P}'}}\right) = \fbox{\py{f'{xazeo:.4f}'}}.
\end{align*}

Peforming a bubble-point pressure calculation at this composition gives the pressure at the azeotrope:
\begin{align*}
    P & = x_1 \gamma_1 P_1^{\rm vap} + x_2 \gamma_2 P_2^{\rm vap}\\
    & = (\py{f'{xazeo.m:.4f}'})(\py{f'{gazeo[0]:.4f}'})(\py{f'{Pv[0]:.1f~L}'}) + (1-\py{f'{xazeo.m:.4f}'})(\py{f'{gazeo[1]:.4f}'})(\py{f'{Pv[1]:.1f~L}'}) = \fbox{\py{f'{Pazeo:.2f~P}'}}.
\end{align*}

\IfFileExists{mypxy-<<<serial>>>-<<<idx>>>.pdf}{
The figure below shows the full $Pxy$ diagram for this binary at \py{f'{T:.2f~P}'}, with the azeotrope labeled.
...
not showing full $Pxy$ diagram for this binary at \py{f'{T:.2f~P}'} yet
}
