...
\begin{pycode}
config = {}
first_ordinal = locals().get('first_ordinal', 'a')
...
if 'content' in SQ and 'config' in SQ:
    ...
    for qdict in questions:
        ...
        if qtype != 'multiple_choice':
            continue
        q_shuffle = qdict.get('shuffle_choices', shuffle_choices)
        q_fixed_last = qdict.get('fixed_last', False)
        if not q_shuffle and not q_fixed_last:
            continue
        correct_choice_text = qdict['choices'][qdict['A']]
        choice_items = list(qdict['choices'].items())
        if q_fixed_last:
            last_item = choice_items[-1]
            texts = [v for _, v in choice_items[:-1]]
            rng.shuffle(texts)
            new_items = [(k, t) for (k, _), t in zip(choice_items[:-1], texts)] + [last_item]
        else:
            texts = [v for _, v in choice_items]
            rng.shuffle(texts)
            new_items = [(k, t) for (k, _), t in zip(choice_items, texts)]
        qdict['choices'] = dict(new_items)
...
if 'AnsSet' in locals() and 'qno' in locals():
    for l, t in zip([chr(ord(first_ordinal)+x) for x in range(len(answers))], answers):
...
\py{instructions}
\begin{pycode}
from pygacity.pythontex.texutils import latex_quotes
...
for iidx, qdict in enumerate(questions):
    ...
    qtext = latex_quotes(qdict['Q'])
    answer = qdict['A']
    explanation = latex_quotes(qdict.get('text', ''))
    qtype = qdict.get('T', global_qtype)
    match qtype:
        case 'short_answer':
        ...
        case 'multiple_choice':
            ...
            for clabel, ctext in qdict['choices'].items():
                print(r'\mcchoiceitem{'+clabel+r'}{'+answer+r'} '+latex_quotes(ctext)+'\n')
            print(r'\end{itemize}'+'\n')
