...
class Config(Yclept):
    def __init__(self, userfile='' ,**kwargs):
        ...
        self.resource_root = resources_dir
    ...
    def _set_external_apps(self, verify_access=True):
        user = self['user'] # expect ycleptic to set this
        paths = user['paths'] # expect ycleptic to set this
        ...
        self.LB = LatexBuilder(paths, localdirs=[self.autoprob_package_dir])
        if verify_access:
    ...
    def _set_internal_shortcuts(self,**kwargs):
        ...
        # self.data_root = self.resource_root / 'data'
        user = self['user']
