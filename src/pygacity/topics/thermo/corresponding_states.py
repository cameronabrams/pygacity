...
class CorrStsChart(ABC):
    ...
    def warp_data(self, indicials_json: Path | str = None):
        ...
        print(f"Loaded {len(pixel_x)} indicials")
    ...
    def _identify_phase_transitions(self, min_jump = 10.0, min_dindep = 0.1, liquid_sense: str ='lower'):
        ...
        for Tr in self.Tr_values:
            ...
            print(f'Tr {Tr}: large jumps: {large_jumps}: {ddep[large_jumps]}')
            if len(large_jumps) == 0:
                # No clear phase transition found
                print(f'No transitions detected by jumps in Hdep at Tr {Tr}')
                self.phase_transitions[Tr] = None
                continue
            elif len(large_jumps) == 3:
                # this is a boomerang -- the first two points should be swapped
                swap_idx = large_jumps[1:]
                print(f'   swapping indices {swap_idx}')
                i, j = swap_idx
                sav = indep[i]
                indep[i] = indep[j]
                indep[j] = sav
            ...
            for i in range(start_idx, jump_idx + 3):
                print(f'{i:>2d} {indep[i]:.3f} {depvar[i]:.3f}')
                if i == jump_idx:
                    print("-"*50)

            if np.abs(indep_after - indep_before) > min_dindep:
                print(f'Tr {Tr}: discretization-jump {dep_before}->{dep_after} indep_before {indep_before} indep_after {indep_after}; no transition')
                self.phase_transitions[Tr] = None
                continue
            ...
            print(f"Tr={Tr:.2f}: Phase transition at {self.indepvar}_sat≈{indep_sat:.3f}")
            print(f"  Discontinuity between indices {jump_idx} ({indep[jump_idx]:.3f}) and {jump_idx+1} ({indep[jump_idx+1]:.3f})")
            print(f"  Liquid: {len(liquid_indices)} points, Hdep: [{liq_depvar.min():.3f}, {liq_depvar.max():.3f}], Pr: [{liq_indep.min():.3f}, {liq_indep.max():.3f}]")
            print(f"  Vapor:  {len(vapor_indices)} points, Hdep: [{vap_depvar.min():.3f}, {vap_depvar.max():.3f}], Pr: [{vap_indep.min():.3f}, {vap_indep.max():.3f}]")
    ...
    def _create_interpolators(self):
        ...
        print(f"{self.depvar}: Created interpolators for {len(self.interpolators)} isotherms")
...
class EntroyDepartureChart(CorrStsChart):
